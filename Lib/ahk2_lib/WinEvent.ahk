 ...
 * `Show`, `Create`, `Close`, `Exist`, `NotExist`, `Active`, `NotActive`, `Move`, 
 * `MoveStart`, `MoveEnd`, `Minimize`, `Restore`, `Maximize`. 
 * See comments for the functions for more information.
 * 
 * All the event initiation methods have the same syntax: 
 * `WinEvent.EventType(Callback, WinTitle:="", Count:=-1, WinText:="", ExcludeTitle:="", 
 *      ExcludeText:="")`
 * 
 * Where Callback is the function that will be called once the event happened, 
 * and Count specifies the maximum amount of callbacks. 
 * `Close` and `NotExist` additionally have the `PropertyCallback` argument as the last one, 
 * which accepts
 *      a window handle and returns property/properties of interest. 
 * Eg `Callback(hWnd) => WinGetTitle(hWnd)`
