...
class HotGestures {
    ...
    ; Содержит свойства:
    ; Gesture массив ключ (массив жеста), и значение это объект.
    __matchInfos := Map()

    ; Время в момент когда совпадения больше не стало ни с какими жестами.
    timeNotMatchWithAnyGestures := false
    ; Таймаут, сколько ждать времени при отсутствии совпадения ни с какими жестами,
    ; и оставлять предыдущее совпадение действительным.
    timeNotMatchWithAnyGesturesTimeout := 200

    __New(maxDistance := 0.1, minTrackLen := 100, penColor := 0xFE4F7F) {
        ; 0.1 - Default значение.
        ; Точность совпадения. 
    ...
    /**
     * Регистрирует жест.
     * @param gesture - Массив жеста (наследует от массива). 
     * @param comment - Название жеста.
     * @param {String} callback - Необязательно.
     */
    Register(gesture, comment, callback := "") {
        matchInfo := { 
            ...
            Comment: comment, 
            Callback: callback, 
            Matrix: HotGestures.DistanceMatrix(gesture), 
            Excluded: false }
    ...
    __OnMouseMove(x, y) {
        ...
        if this.__trackLen < this.MinTrackLength
            ...
            ; Длина начальной нарисованной кривой достаточно большая, 
            ; чтобы начать сравнивать с жестами.
            ; if(!this.__match){
            ;     Debug().logF("__UpdateMatch(), this.__match = ")
            ; }
            
            tip := ""
            ; Если есть совпадение с каким-то жестом.
            ; Строим текст tip для отображения названия жеста на верху экрана.
            if this.__match {
                ; Если нет совпадения ни с одним жестом, то это this.__match будет
                ; false. 

                ; Текст в начале, до символа : в жесте.
                name := this.__match.Gesture.Name
                ; Название жеста, указанное в функции Register()
    ...
    ; Вызывается при перемещении курсора, чтобы проверить на совпадение
    ; с жестами.
    ; Это должно возвращать true, если было совпадение с каким-то новым жестом.
    ; и false, если было совпадение с активным жестом.
    __UpdateMatch() {
        match := ""
        ...
        if(this.__match != "" && match == ""){
            ; Сейчас нет ни одного совпадения с жестами.
            ; Включим счётчик секунд с этого времени.
            ; Оставим совпадение с преждним жестом.
            if(this.timeNotMatchWithAnyGestures == false){
                this.timeNotMatchWithAnyGestures := A_TickCount
            }
            if(A_TickCount - this.timeNotMatchWithAnyGestures < this.timeNotMatchWithAnyGesturesTimeout){
                ; Таймаут ещё не закончился.
                return false
            }else{
                ; Таймаут закончился.
                this.timeNotMatchWithAnyGestures := false
                this.__match := match
                return true
            }
        }
        ...
        if(match != ""){
            ; Есть какое-то совпадение с жестом.
            this.timeNotMatchWithAnyGestures := false
        }

        if(this.__match == match){
            ; Это совпадение match такое же как предыдущее this.__match.
            return false
        }
    ...
    /**
     * Вызывается статически:
     * HotGestures.Gesture()
     * Создаёт жест.
     * Это массив. Это наследует от массива.
     * И добавляет свойство name к массиву.
     */
    class Gesture extends Array {

        /**
         * Это может принимать или один аргумент, строку жеста: "имя жеста:стока жеста".
         * Или два аргумента: имя жеста, и массив векторов жеста.
         * @param name 
         * @param vectors 
         */
        __New(name, vectors?) {
        ...
        ; Переводит массив жеста в строку. Без имени жеста в начале.
        ToString() {
            vectorsStr := ""
            for v in this
        ...
        /**
         * Заполняет переменную copy, массивом векторов жеста, из массива 
         * векторов vectorsArr.
         * Убеждается что массив векторов vectorsArr правильный.
         * И возвращает true если правильный, иначе false. 
         * @param vectorsArr 
         * @param copy 
         * @returns {Integer} 
         */
        ...
        /**
         * Заполняет 2, 3 аргумент значениями.
         * @param vectorsStr - Строка жеста: название:строка жеста.
         * @param name - Название жеста.
         * @param vectorsArr - Массив векторов, из жеста.
         */
    ...
    /**
     * Это массив. Наследует от массива.
     * Добавляет свойства:
     */
    class DistanceMatrix extends Array {

        /**
         * Содержит массив расстояний.
         * Первый элемент содержит массив из бесконечных значений.
         *  Число этих значений равно числу элементов массива жеста.
         * Следующие элементы - это массив векторов расстояния.
         */

        /**
         * Вызывается в методе Register().
         * @param standard - Массив жеста, объект Gesture.
         */
        __New(standard) {
            ; NumGet - Returns the binary number stored at the specified address+offset.
            
            ; For example, 0x7F800000 is positive infinity, and 0xFF800000 is negative infinity.
            ; A_PtrSize - Contains the size of a pointer, in bytes. 
            ; This is either 4 (32-bit) or 8 (64-bit), depending on what type of executable (EXE) is running the script.
            static INF := NumGet(ObjPtr(&_ := 0x7F800000) + A_PtrSize * 2, "float")

            if !len := standard.Length
                throw Error("invalid parameter")

            ; Массив жеста, объект Gesture.
            this.Standard := standard
            ; Длина массива жеста + 1.
            this.Capacity := len + 1

            ; Массив, содержит бесконечное значение в первом элементе.
            this.RowTemplate := []
        ...
        /**
         * Добавляет новый элемент в массив этого объекта.
         * Массив расстояний.
         * @param newVector 
         */
        Append(newVector) {
            /**
             * Определяется функция Distance().
             * Определяет расстояние между поступившим вектором, 
             * и вектором из этого массива жеста, объекта Gesture.
             * @param a 
             * @param b 
             * @returns {Number} 
             */
        ...
        /**
         * Возвращает число.
         * Возможно которое определяет близость соответствия. 
         * Чем меньше значение, тем больше соответствие.
         * @returns {Number} 
         */
        TraceBack() {
        ...
        /**
         * Обнуляет этот массив объект расстояний. 
         * @returns {Integer} 
         */
        Clear() => this.Length := 1
    }
